...
- AI 根据选中图片、内容、评论和补充要求生成 Cover、手账文字及零张或多张贴纸。用户补充位于生成上下文最前方，并标记为文字、Cover 与贴纸都应优先遵循的最高优先级要求；图片优先发送缩略图，缩略图不存在时回退发送原图。
...
- 再次点击已有手账的日期时，其余界面逐渐模糊但不压暗，日历格内显示加载状态；大图就绪后，缩略图从原位放大为只含手账图片的展示卡片，“编辑”和“删除”按钮随后从卡片下方掉出；点击编辑后才进入素材与画布界面。
