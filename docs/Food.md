...
所有已登录账号都可以修改或删除任意美食组。修改面板支持新增、替换和删除单张历史图片，每组始终保留 1～12 张；详情卡外侧可切换上一组、下一组，组内缩略图继续用于切换同组图片。整组删除前先记录精确相对路径并把组切换为不可见的草稿状态，再按实际来源删除全部本地文件或旧 Storage 对象以及数据库记录；若文件清理失败，应恢复可见状态并向用户报告错误。上传或图片编辑失败必须清理本次产生的文件。
...
### 14.1 动画实现与性能审查（2026-08-19）

- Food 与 Photos 共用 `src/lib/motion.ts` 中的动画节奏：快速反馈 160ms、常规过渡 300ms、布局变形 420ms，主要缓动为 `cubic-bezier(0.22, 1, 0.36, 1)`。
- 画廊展开继续使用 FLIP，但会先读取正在播放动画的当前屏幕位置，再取消旧动画并衔接新目标；快速操作不会先跳到旧终点。布局读操作分布在状态更新前后，实际播放只修改 `transform`。
- 卡片仅在 FLIP 播放期间临时设置 `will-change: transform`，结束、中断和卸载都会清除；不再让每张图片长期占用独立 GPU 合成层。
- 普通卡片悬停不再动画化 `box-shadow`，避免每帧阴影重绘；只有精细指针设备使用轻微上浮。触摸长按会先给出 0.985 的压感反馈，滑动超过阈值立即取消。
- 统计面板同时动画化折叠轨道、透明度和位移，打开与收起均有连续状态；新增和修改 Dialog 使用 300ms 进入、200ms 退出与遮罩淡入淡出。
- Reduced Motion 下 FLIP、翻面、统计开合、Dialog、平滑滚动和计时滚动都会压缩为近乎即时状态；功能、焦点和布局保持不变。
- production Chrome 用 8 张不同比例的本地假图验收：桌面和 390×844 移动视口的连续切卡与统计开合 P95 均约 16.8ms，没有超过 25ms 的帧，动画结束后活动 `will-change` 数量为 0，浏览器运行时错误为 0。假数据、测试路由、浏览器配置和测试进程均已清理，未访问正式数据库。
