...
一个作品需要展示多张图片时，将 `coverFile` 写成数组，最多 8 个媒体项目：
...
coverFile: [
  ...
  "personal-knowledge-base-04.webp",
],
```

上例会依次显示一张通栏图片、一行两张图片、另一张通栏图片。子数组中的图片会保持原始比例并自动分配宽度，使它们具有相同高度、底边对齐，并让整行左右边缘与通栏图片对齐。子数组不能为空；最多 8 项的限制按所有内外层媒体的总数计算。
...
数组顺序就是画廊顺序，图片文件名从服务器本地读取，视频链接使用 B 站播放器。
...
- 一个作品最多填写 8 个图片或视频项目；视频可以单独使用，也可以和图片、其他 B 站视频混合；
...
| `title` | 是 | 首页显示的作品标题。 |
| `description` | 否 | 作品简介。 |
| `coverFile` | 否 | 一个媒体项目或媒体数组；顶层项目纵向排列，子数组中的项目横向排列。所有内外层媒体合计最多 8 项，每项可以是本地图片文件名或完整的 HTTPS B 站视频链接。 |
| `tags` | 否 | 技术或主题标签数组。 |
...
- 作品 ID 是否符合格式且没有重复；
- 标题和描述是否包含意外的首尾空格；
- 图片项目是否只是安全文件名、扩展名是否受支持且没有重复；
- `coverFile` 顶层数组和横向行子数组是否为空；
- 视频项目是否为受支持的完整 HTTPS B 站链接；
- 媒体列表是否超过 8 项；
