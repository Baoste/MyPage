...
export default async function CalendarPage({ searchParams }: CalendarPageProps) {
  ...
  try { days = await getCalendarMonth(session.userId, monthKey); } catch (error) { unavailable = error instanceof Error ? error.message : "日历数据暂时不可用。"; }
  ...
  return <section className={styles.page} aria-labelledby="calendar-title"><div className={`container-shell ${styles.workspace}`}>
    ...
    {unavailable ? <p className={styles.notice}>{unavailable}</p> : null}<p className={styles.scrollHint}>横向滑动查看完整月份</p>
    <div className={styles.journalLayout}><aside className={styles.notesPanel} aria-label="本月手账留白"><p className={styles.notesLabel}>Notes</p><p className={styles.notesCopy}>留住有照片、有味道的日子。</p><div className={styles.notesLines} aria-hidden="true" /></aside><CalendarExperience year={displayed.year} month={displayed.month} today={today} initialDays={days} /></div>
  </div></section>;
}
