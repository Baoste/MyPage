...
import {
  CALENDAR_MAX_IMAGES, CALENDAR_MAX_MONTH_NOTE_LENGTH, CALENDAR_MAX_NOTE_LENGTH, CALENDAR_MAX_SOURCES, CALENDAR_MAX_TEXT_LENGTH, CALENDAR_TIMEZONE,
...
async function entriesWithAssets(start: string, end: string) {
  const client = createServerSupabaseClient();
  const { data, error } = await client.from("calendar_entries").select("id,owner_user_id,entry_date,status,user_note,generated_text,final_text,layout_json,generation_meta,updated_at,last_error").gte("entry_date", start).lt("entry_date", end);
...
export async function getCalendarMonth(month: string): Promise<CalendarMonthDay[]> {
  ...
  const [photos, foods, entries] = await Promise.all([
    ...
    entriesWithAssets(startDate, endDate),
  ]);
...
export async function getCalendarMonthNote(month: string) {
  ...
  return data ? { content: data.content as string, updatedAt: data.updated_at as string } : null;
}

export async function saveCalendarMonthNote(userId: string, month: string, content: unknown) {
  if (!isCalendarMonth(month)) throw new CalendarServiceError("月份格式无效。", 400);
...
async function getEntry(date: string) { return (await entriesWithAssets(date, nextDate(date)))[0] ?? null; }

export async function getCalendarDay(date: string): Promise<CalendarDayPayload> {
...
export async function generateEntry(userId: string, date: string, sourceIds: string[], imageIds: string[], userNote: string) {
  ...
  const day = await getCalendarDay(date); const selected = day.sources.filter((source) => sourceIds.includes(`${source.type}:${source.id}`));
  ...
  const client = createServerSupabaseClient();
  const previous = await client.from("calendar_entries").select("*").eq("entry_date", date).maybeSingle();
  ...
  const entryOwnerId = previous.data?.owner_user_id ?? userId;
  const { data: entryData, error: entryError } = await client.from("calendar_entries").upsert({ owner_user_id: entryOwnerId, entry_date: date, timezone: CALENDAR_TIMEZONE, status: "generating", user_note: userNote.trim(), source_manifest: manifest, generation_meta: { stage: "preparing" }, last_error: null }, { onConflict: "entry_date" }).select("id").single();
  ...
  try {
    ...
    await client.from("calendar_entries").update({ status: "draft", generated_text: result.text, final_text: result.text, layout_json: layout, generation_meta: result.meta, last_error: null }).eq("id", entryData.id);
    ...
    if (previous.data) {
      ...
      await client.from("calendar_entries").update({ status: old.status, user_note: old.user_note, generated_text: old.generated_text, final_text: old.final_text, source_manifest: old.source_manifest, layout_json: old.layout_json, generation_meta: old.generation_meta, last_error: lastError }).eq("id", entryData.id);
    } else {
      await client.from("calendar_entries").update({ status: "failed", last_error: lastError }).eq("id", entryData.id);
    }
...
export async function saveEntry(id: string, finalText: unknown, layoutValue: unknown, updatedAt: unknown) {
  ...
  let query = client.from("calendar_entries").update({ final_text: finalText.trim(), layout_json: layout, status: "ready", last_error: null }).eq("id", id);
  ...
  return getEntry(data.entry_date);
}

export async function savePreview(id: string, bytes: ArrayBuffer, mimeType: string, role: "preview" | "thumbnail" = "preview") {
  ...
  const client = createServerSupabaseClient(); const entry = await client.from("calendar_entries").select("id,owner_user_id").eq("id", id).maybeSingle();
  ...
  const assetId = randomUUID(), extension = role === "preview" ? "png" : "webp", path = `calendar/${entry.data.owner_user_id}/${id}/${assetId}.${extension}`; await uploadPrivateAsset(path, bytes, mimeType);
...
export async function readCalendarAsset(id: string) {
  const client = createServerSupabaseClient(); const { data, error } = await client.from("calendar_assets").select("storage_path,mime_type,byte_size").eq("id", id).maybeSingle();
...
export async function deleteEntry(id: string) {
  const client = createServerSupabaseClient(); const assets = await client.from("calendar_assets").select("storage_path").eq("calendar_entry_id", id);
  const { data, error } = await client.from("calendar_entries").delete().eq("id", id).select("id").maybeSingle();
