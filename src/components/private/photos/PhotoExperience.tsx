  ...
  return (
    <div className="pb-28">
      <div ref={statsRef} className={`scroll-mt-28 ${statsOpen ? "container-shell pt-6 md:pt-8" : ""}`}>
        {statsOpen ? <PhotoStatsPanel statistics={statistics} /> : null}
      </div>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-[max(1rem,env(safe-area-inset-bottom))] z-30 flex items-end justify-between px-4 sm:px-6">
        <button type="button" aria-expanded={statsOpen} aria-controls="photo-statistics-region" onClick={toggleStatistics} className="pointer-events-auto grid size-14 place-items-center rounded-full bg-[#30342e] text-[0.62rem] font-semibold tracking-[0.08em] text-white shadow-[0_10px_30px_rgba(30,30,25,0.2)] transition-transform hover:-translate-y-0.5">
          {statsOpen ? "收起" : "统计"}
        </button>
        <button type="button" disabled={!uploadEnabled} aria-describedby={!uploadEnabled ? "photo-upload-unavailable" : undefined} onClick={() => setUploadOpen(true)} aria-label="新增照片" title={uploadEnabled ? "新增照片" : uploadDisabledReason ?? "暂时无法上传"} className="pointer-events-auto grid size-14 place-items-center rounded-full bg-[#a64b2a] text-3xl font-light leading-none text-white shadow-[0_10px_30px_rgba(88,44,27,0.24)] transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-45">
