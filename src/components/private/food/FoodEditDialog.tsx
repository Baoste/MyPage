    ...
    >
      <form onSubmit={submit} className="flex h-full min-h-0 flex-col">
        <header className="flex items-start justify-between gap-6 border-b border-[#cec5b8] px-5 py-5 sm:px-8">
          ...
          <button type="button" disabled={isSaving} onClick={requestClose} aria-label="关闭修改美食记录" className="grid size-11 shrink-0 place-items-center border border-[#bcb3a8] text-xl disabled:opacity-40">×</button>
        </header>
        ...
        <div className="min-h-0 flex-1 space-y-7 overflow-y-auto px-5 py-6 sm:px-8">
          <div className="grid gap-5 sm:grid-cols-[1fr_auto]">
            <label className="block text-xs font-semibold text-[#5d554e]">
              分类
              <input value={category} onChange={(event) => { setCategory(event.target.value); setIsDirty(true); }} disabled={isSaving} required maxLength={40} className="mt-2 w-full border border-[#bdb3a7] bg-[#fbf8f2] px-3 py-2.5 text-sm font-normal text-[#302d29]" />
            </label>
          ...
          <label className="block text-xs font-semibold text-[#5d554e]">
            发生时间
            <input type="datetime-local" value={occurredAtLocal} onChange={(event) => { setOccurredAtLocal(event.target.value); setIsDirty(true); }} disabled={isSaving} required className="mt-2 w-full border border-[#bdb3a7] bg-[#fbf8f2] px-3 py-2.5 text-sm font-normal text-[#302d29]" />
            <span className="mt-1 block text-[0.66rem] font-normal leading-5 text-[#7b736a]">固定为中国北京时间（{FOOD_TIMEZONE}）</span>
          </label>

          <label className="block text-xs font-semibold text-[#5d554e]">
            点评
            <textarea value={review} onChange={(event) => { setReview(event.target.value); setIsDirty(true); }} disabled={isSaving} maxLength={2000} rows={5} className="mt-2 w-full resize-y border border-[#bdb3a7] bg-[#fbf8f2] px-3 py-2.5 text-sm font-normal leading-6 text-[#302d29]" />
        ...
        <footer className="flex flex-wrap items-center justify-between gap-4 border-t border-[#cec5b8] bg-[#eee8de] px-5 py-4 sm:px-8">
          ...
          <button type="submit" disabled={isSaving} className="min-w-32 bg-[#2e332c] px-5 py-3 text-xs font-semibold tracking-[0.1em] text-white disabled:opacity-50">
            {isSaving ? "保存中…" : "保存修改"}
          </button>
        </footer>
      </form>
