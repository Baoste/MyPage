        ...
        <ol className="mt-4 space-y-3">
          {items.map((item) => (
            <li key={item.key}>
              ...
              <div className="mt-1.5 h-1.5 overflow-hidden bg-[#ddd5c9]" aria-hidden="true">
                <div className="h-full bg-[#8f4d34]" style={{ width: `${item.percentage}%` }} />
              </div>
            </li>
          ))}
        </ol>
...
export function FoodStatsPanel({ statistics }: { statistics: FoodStatistics }) {
  ...
  return (
    <section id="food-statistics-region" aria-labelledby="food-stats-title" className="food-stats-enter border-y border-[#c9c0b4] bg-[#eee8de] px-5 py-8 sm:px-8 sm:py-10">
      ...
      <dl className="mt-8 grid grid-cols-2 gap-px border border-[#c9c0b4] bg-[#c9c0b4] sm:grid-cols-3 lg:grid-cols-6">
        {[
          ...
          <div key={label} className="bg-[#f5f0e8] px-4 py-5 text-center">
          ...
          <div className="mt-5 grid h-36 grid-cols-12 items-end gap-1" role="img" aria-label={`最近十二个月共记录 ${statistics.monthlyTimeline.reduce((total, item) => total + item.count, 0)} 组美食`}>
            {statistics.monthlyTimeline.map((item) => (
              <div key={item.key} className="flex h-full min-w-0 flex-col justify-end gap-2 text-center">
                ...
                <span className="block min-h-1 bg-[#9b5b40]" style={{ height: `${Math.max(4, (item.count / maximumMonth) * 88)}px` }} aria-hidden="true" />
        ...
        <section className="mt-10 border-t border-[#c9c0b4] pt-8">
          ...
          <ul className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {statistics.todayMemories.map((memory) => (
              <li key={memory.id} className="border border-[#c9c0b4] bg-[#f5f0e8] p-4 text-sm">
