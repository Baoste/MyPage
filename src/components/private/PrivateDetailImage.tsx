...
type OriginalImageStatus = "idle" | "loading" | "revealing" | "loaded" | "error";
...
export function PrivateDetailImage({
  ...
  const originalRequested = originalStatus === "loading"
    || originalStatus === "revealing"
    ...
    <>
      ...
      {canLoadOriginal && originalRequested ? (
        <Image
          ...
          fill
          sizes={sizes}
          className={`${imageClassName} z-[1] ${
            originalStatus === "revealing"
              ? "private-original-image-reveal"
              : originalStatus === "loaded"
                ? "visible"
                : "invisible"
          }`}
          ...
          fetchPriority="high"
          loading="eager"
          onLoad={() => setOriginalStatus("revealing")}
          onError={() => setOriginalStatus("error")}
          onAnimationEnd={() => {
            setOriginalStatus((current) => current === "revealing" ? "loaded" : current);
          }}
        />
      ) : null}
